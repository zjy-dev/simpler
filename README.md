...
把厚书读薄。技术书砍掉废话，只留面试要考的，输出 Markdown + PDF。
...
```text
source/<书名>/        ← 原始素材（Markdown、图片等）
simplified/<书名>/    ← 精简后的 Markdown
simplified/*-simplified.pdf  ← 生成的 PDF
scripts/build_pdf.py  ← PDF 生成脚本
.opencode/agents/     ← AI 代理配置（简化工作流）
.opencode/skills/     ← AI 技能包（PDF 提取、去 AI 味等）
```
...
输出到 `simplified/<书名>-simplified.pdf`。

### 添加新书

1. 把原始素材放到 `source/<书名>/`
2. 精简内容，输出到 `simplified/<书名>/`，文件命名 `<书名>-<部分>-<章>.md`
3. 在 `scripts/build_pdf.py` 的 `BOOKS` 字典里加一条配置
4. 跑 `build_pdf.py <书名>` 生成 PDF

## 例子：MySQL 是怎样运行的

原书 27 章，砍掉阅读指南、字符集、数据目录、optimizer trace 等非面试内容（9 章），剩下的合并、精简，最终分成三个部分 + 一个命令速查：
...
├── mysql-02-04.md   ← EXPLAIN 详解（两章合并为一章）
├── mysql-03-01.md   ← 事务简介
├── mysql-03-02.md   ← redo 日志（两章合并为一章）
├── mysql-03-03.md   ← undo 日志（两章合并为一章）
...
浅黄底色 + 天蓝标题 + 红色重点标注。封面、可点击目录、分部分隔页、自动分页。用文泉驿微米黑渲染中文，NotoSansMono 渲染代码。

## AI 辅助

用 [opencode](https://github.com/opencode-ai/opencode) 的 agent 跑简化流程。agent 配置在 `.opencode/agents/simpler.md`，技能包在 `.opencode/skills/` 下（PDF/DOCX/PPTX/XLSX 提取、`humanizer-zh` 去 AI 味）。
